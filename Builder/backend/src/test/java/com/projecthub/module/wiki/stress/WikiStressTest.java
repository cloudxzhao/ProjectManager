 ...
 *
 * 使用方法:
 * 1. 确保后端服务已启动 (http://localhost:9527)
 * 2. 确保数据库和 Redis 已启动
 * 3. 运行此测试类
 *
 * 测试内容:
...
public class WikiStressTest {
    ...
    void setUp() {
        ...
        UserDetailsImpl testUser = new UserDetailsImpl(
            testUserId, "admin", "admin@projecthub.com", "password", null,
            true, true, true, true);
    ...
    public void stressTest_Create100FlatDocuments() {
        ...
        for (int i = 1; i <= 100; i++) {
            WikiVO.CreateRequest request = WikiVO.CreateRequest.builder()
                .title("测试文档-" + i)
    ...
    /**
     * 压力测试 2: 创建 100 条树形结构的 Wiki 文档 (3 层结构)
     * 结构：5 个根节点 -> 15 个二级节点 -> 75 个三级节点 + 5 个独立文档 = 100 条
     */
    @Test
    ...
    public void stressTest_CreateDeepChain() {
        ...
        for (int i = 1; i <= 10; i++) {
            WikiVO.CreateRequest request = WikiVO.CreateRequest.builder()
                ...
                .content("# 第 " + i + " 层节点\n\n" +
                        "这是深度测试的第 " + i + " 层节点。\n" +
                        "父节点：" + (parentId != null ? parentId : "无 (根节点)") + "\n" +
                        "创建时间：" + LocalDateTime.now())
                .build();
    ...
    public void stressTest_VerifyTreeStructure() {
        ...
        // 先创建一批测试数据
        WikiVO root1 = wikiService.createDocument(testProject.getId(),
            WikiVO.CreateRequest.builder().title("验证测试 - 根 1").content("根节点 1").build());
        WikiVO root2 = wikiService.createDocument(testProject.getId(),
            WikiVO.CreateRequest.builder().title("验证测试 - 根 2").content("根节点 2").build());

        for (int i = 1; i <= 5; i++) {
            wikiService.createDocument(testProject.getId(),
                WikiVO.CreateRequest.builder().parentId(root1.getId())
                    .title("根 1-子节点-" + i).content("内容").build());
            wikiService.createDocument(testProject.getId(),
                WikiVO.CreateRequest.builder().parentId(root2.getId())
                    .title("根 2-子节点-" + i).content("内容").build());
        }
        ...
        for (WikiVO root : tree) {
            System.out.println("根节点：" + root.getTitle() +
                             ", 子节点数：" + (root.getChildren() != null ? root.getChildren().size() : 0));
        }
    ...
    private void verifyDepth(WikiVO node, int currentDepth) {
        ...
        System.out.println(indent + "└─ 第" + currentDepth + "层：" + node.getTitle() + " (ID: " + node.getId() + ")");
