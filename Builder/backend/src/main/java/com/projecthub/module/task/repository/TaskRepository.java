...
public interface TaskRepository extends JpaRepository<Task, Long>, JpaSpecificationExecutor<Task> {
     ...
     * 查询项目下的任务列表
     */
    @Query("SELECT t FROM Task t WHERE t.projectId = :projectId AND t.deletedAt IS NULL " +
     ...
     * 根据负责人查询任务
     */
    @Query("SELECT t FROM Task t WHERE t.assigneeId = :assigneeId AND t.deletedAt IS NULL " +
            "ORDER BY t.createdAt DESC")
     ...
     * 查询项目下指定状态的任务
     */
    @Query("SELECT t FROM Task t WHERE t.projectId = :projectId AND t.status = :status AND t.deletedAt IS NULL " +
            "ORDER BY t.position ASC")
